...
namespace Calendar_by_I_M_Marinov.Services
{
    ...
    public class GooglePeopleService: IGooglePeopleService
    {
		...
		public async Task<Person> UpdateContactAsync(string resourceName, ContactViewModel updatedContact)
		{
			...
			{
				if (DateTime.TryParse(updatedContact.Birthday, out DateTime parsedDate))
				{
				...
				{
					Console.WriteLine("Invalid date format for birthday.");
				}
			}
			else
			{
				personToUpdate.Birthdays = null; // Set to null if no birthday provided
			}
		...
		private List<ContactViewModel> SearchContactsInMemory(string searchTerm, List<ContactViewModel> allContacts)
		{
			searchTerm = searchTerm.ToLower(); // case-insensitive comparison

			var searchResults = allContacts.Where(contact =>
				...
				(!string.IsNullOrEmpty(contact.Email) && contact.Email.ToLower().Contains(searchTerm)) ||
				(!string.IsNullOrEmpty(contact.PhoneNumber) && contact.PhoneNumber.Contains(searchTerm))
			).ToList(); 

			return searchResults;
		}
		public async Task<List<ContactViewModel>> SearchContactsAsync(string text, int page = 1)
		{
			var allContacts = await GetAllContactsAsync();

			var searchResults = SearchContactsInMemory(text, allContacts);
